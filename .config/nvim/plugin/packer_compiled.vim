...
local plugins = {
  ...
  ["completion-treesitter"] = {
    ...
    only_sequence = false,
    only_setup = false,
    path = "/home/brian/.local/share/nvim/site/pack/packer/opt/completion-treesitter"
  },
  ["emmet-vim"] = {
    loaded = false,
    only_sequence = false,
...
augroup packer_load_aucmds
  au!
  " Filetype lazy-loads
  au FileType lua ++once call s:load(['nvim-treesitter', 'vim-endwise', 'BetterLua.vim'], { "ft": "lua" })
  au FileType html ++once call s:load(['vim-closetag', 'emmet-vim', 'bracey.vim'], { "ft": "html" })
  au FileType rust ++once call s:load(['nvim-treesitter', 'vim-test', 'splitjoin.vim'], { "ft": "rust" })
  ...
  au FileType markdown ++once call s:load(['nvim-treesitter', 'vim-markdown', 'markdown-preview.nvim'], { "ft": "markdown" })
  au FileType vim ++once call s:load(['vim-endwise', 'vim-test', 'splitjoin.vim'], { "ft": "vim" })
  au FileType plaintex ++once call s:load(['vimtex'], { "ft": "plaintex" })
  au FileType python ++once call s:load(['nvim-treesitter', 'vim-test', 'splitjoin.vim'], { "ft": "python" })
  au FileType ruby ++once call s:load(['vim-endwise'], { "ft": "ruby" })
  au FileType css ++once call s:load(['emmet-vim', 'bracey.vim'], { "ft": "css" })
  " Event lazy-loads
  au InsertEnter * ++once call s:load(['vim-vsnip-integ', 'completion-treesitter', 'completion-buffers', 'vim-vsnip', 'completion-nvim'], { "event": "InsertEnter *" })
augroup END
